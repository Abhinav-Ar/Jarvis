...
# Deploy the same src/package layout used in development.
for file in orion.py jarvis.py requirements.txt start.sh; do
  /usr/bin/ditto "$PROJECT_DIR/$file" "$APP_DIR/$file"
done
/usr/bin/ditto "$PROJECT_DIR/src" "$APP_DIR/src"
/usr/bin/ditto "$PROJECT_DIR/scripts" "$APP_DIR/scripts"

# Remove the pre-layout flat runtime after the packaged source tree is present.
for legacy in orion_kernel.py orion_replay.py generation.py capability_families.py anticipation.py design_intelligence.py google_workspace.py app_installer.py project_workspace.py blender_worker.py blender_advanced_worker.py blender_existing_worker.py freecad_worker.py openscad_worker.py resolve_worker.py assist.py audio.py activity.py diagnostics.py fast_commands.py tools.py spot.py mac_tools.py integrations.py desktop.py git_tools.py task_engine.py agent_platform.py project_workflow.py execution_engine.py execution_supervisor.py recovery.py; do
  rm -f "$APP_DIR/$legacy"
done
...
/usr/bin/swiftc \
  ...
  -framework Vision \
  "$PROJECT_DIR/packaging/macos/JarvisHUDView.swift" \
  "$PROJECT_DIR/packaging/macos/JarvisMenu.swift" \
  -o "$MENU_APP/Contents/MacOS/JarvisMenu"
/usr/bin/swiftc \
  -module-cache-path "$APP_DIR/.swift-cache" \
  "$PROJECT_DIR/packaging/macos/JarvisDesktopHelper.swift" \
  -o "$MENU_APP/Contents/MacOS/JarvisDesktopHelper"
/usr/bin/ditto "$PROJECT_DIR/packaging/macos/JarvisMenu-Info.plist" "$MENU_APP/Contents/Info.plist"
/usr/bin/codesign --force --deep --sign - "$MENU_APP"

sed "s|__PROJECT_DIR__|$APP_DIR|g" "$PROJECT_DIR/packaging/macos/$LABEL.plist" > "$TARGET"
sed -e "s|__PROJECT_DIR__|$APP_DIR|g" -e "s|__MENU_APP__|$MENU_APP|g" \
  "$PROJECT_DIR/packaging/macos/$MENU_LABEL.plist" > "$MENU_TARGET"
plutil -lint "$TARGET"
